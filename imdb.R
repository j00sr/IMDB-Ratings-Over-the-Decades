...
tens_plot <- tens_plot + theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
tens_plot <- tens_plot + labs(title = "2010s", x = "Year of Release", y = "Rating")
tens_plot

# for 2000-2010
zeroes_plot <- ggplot(zeroes, aes(x = factor(title_year), y = imdb_score)) + geom_boxplot()
zeroes_plot <- zeroes_plot + theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
zeroes_plot <- zeroes_plot + labs(title = "2000s", x = "Year of Release", y = "Rating")
zeroes_plot

# for 1990-2000
nineties_plot <- ggplot(nineties, aes(x = factor(title_year), y = imdb_score)) + geom_boxplot()
nineties_plot <- nineties_plot + theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
nineties_plot <- nineties_plot + labs(title = "1990s", x = "Year of Release", y = "Rating")
nineties_plot

# for 1980-1990
eighties_plot <- ggplot(eighties, aes(x = factor(title_year), y = imdb_score)) + geom_boxplot()
eighties_plot <- eighties_plot + theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
eighties_plot <- eighties_plot + labs(title = "1980s", x = "Year of Release", y = "Rating")
eighties_plot
